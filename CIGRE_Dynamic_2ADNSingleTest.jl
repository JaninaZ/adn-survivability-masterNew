...
# this part does not differ much as before, but change the P_limit of the 2nd ADN
    ...
    P_ref(t) = t > 0.25 ?  0.8*real(S_total) : real(S_total)  # try 0.1 or something small, vary the 1.1 here to see the change of P_g or P_int
    Q_ref(t) = t > 0.25 ?  imag(S_total)-2 : imag(S_total) 
...
savefig("$dir/simulationplots/CIGRE_Dynamic_2ADN_PerrSingle.png")
# plot!(sol.t, Perr)
...
savefig("$dir/simulationplots/CIGRE_Dynamic_2ADN_VoltageAbsoluteSingle.png")
...
plot(sol, vars=2)# slack bus 
plot!(sol, vars=4)#DG unit at node 2 and so on 
plot!(sol, vars=11)
plot!(sol, vars=18)
plot!(sol, vars=25)
plot!(sol, vars=32)
...
plot!(sol, vars=67)
plot!(sol, vars=74)
savefig("$dir/simulationplots/CIGRE_Dynamic_2ADN_voltageSingleImag.png")
